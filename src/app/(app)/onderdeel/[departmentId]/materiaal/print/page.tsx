  ...
  return (
    <div className="px-4 pt-4">
      <div className="no-print mb-4 flex items-center justify-between">
        <p className="text-[13px] text-text-muted">{labels.length} labels op dit printvel</p>
        <PrintButton />
      </div>

      {labels.length === 0 ? (
        <p className="text-text-muted">Geen materiaal gevonden voor dit printvel.</p>
      ) : (
        <div className="grid grid-cols-3 gap-3 print:grid-cols-3">
          {labels.map(({ material, qrDataUrl }) => (
            <div
              key={material.id}
              className="flex flex-col items-center break-inside-avoid rounded-lg border border-dashed border-card-border p-2 text-center"
            >
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src={qrDataUrl} alt={material.id} width={90} height={90} />
              <p className="font-display mt-1 text-[13px] font-extrabold italic text-ink">{material.id}</p>
