...
export function ScanView({
  ...
  const open = useCallback(
    async (ruwId: string) => {
      ...
      setOnbekendId(null);
      try {
        const { data, error } = await actions.zoekMateriaalId({ onderdeelId, ruwId });
        const gevonden = error ? null : data.materiaalId;
        if (!gevonden) {
        ...
        window.location.assign(`/${slug}/materiaal/${encodeURIComponent(gevonden)}`);
      } catch {
        setOnbekendId(ruwId.trim().toUpperCase());
        setBezig(false);
        bezigRef.current = false;
