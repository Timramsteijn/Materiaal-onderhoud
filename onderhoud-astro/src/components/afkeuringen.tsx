 ...
 */
export function AfkeuringenKaart({
  aanvragen,
  goedkeurUrl,
  afwijsUrl,
}: {
  ...
  async function beslis(
    event: ReactSubmitEvent<HTMLFormElement>,
    actie: typeof actions.keurAfkeuringGoed | typeof actions.wijsAfkeuringAf
  ) {
    event.preventDefault();
    setBezig(true);
    await actie(new FormData(event.currentTarget));
        ...
        <ul className="mt-3 divide-y divide-zand">
          {aanvragen.map((a) => (
            <li key={a.id} className="py-3 first:pt-0 last:pb-0">
              ...
              {a.reden && (
                <p className="mt-1 text-[13px] text-text-medium">&ldquo;{a.reden}&rdquo;</p>
              )}
              ...
              <div className="mt-2.5 flex flex-wrap gap-2.5">
                <form
                  method="post"
                  action={goedkeurUrl}
                  onSubmit={(e) => beslis(e, actions.keurAfkeuringGoed)}
                >
                  <input type="hidden" name="materiaalDbId" value={a.id} />
                  <button
                    type="submit"
                    disabled={bezig}
                ...
                <form
                  method="post"
                  action={afwijsUrl}
                  onSubmit={(e) => beslis(e, actions.wijsAfkeuringAf)}
                >
                  <input type="hidden" name="materiaalDbId" value={a.id} />
                  <button
                    type="submit"
